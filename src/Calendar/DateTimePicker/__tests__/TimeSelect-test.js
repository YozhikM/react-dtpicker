...
describe('TimeSelect', () => {
  ...
  describe('parseTime()', () => {
    ...
    const parsedTime = wrapper.instance().parseTime(value);

    expect(parsedTime).toEqual({ hh: 10, mm: 9, ss: 8 });
  });
  ...
  describe('user interactions', () => {
    it('onSubmit()', () => {
      ...
      const wrapper = shallow(<TimeSelect value={value} onSubmit={onSubmit} />);
      findSubmitBtn(wrapper).simulate('click');

      expect(onSubmit).toBeCalled();
      const callbackValue = onSubmit.mock.calls[0][0];
      // check immutability
    ...
    it('onChange()', () => {
      ...
      const wrapper = shallow(<TimeSelect value={value} onSetTime={onSetTime} />);
      wrapper.instance().onSetTime();

      expect(onSetTime).toBeCalled();
    });

    it('onChangeHH', () => {
      const dt = new Date();
      const onSetTime = jest.fn();
      const wrapper = shallow(<TimeSelect value={dt} onSetTime={onSetTime} />);
      wrapper
        .find('InputUpDown')
        .at(0)
        .props()
      ...
      expect(onSetTime).toBeCalledWith(setHours(dt, 14));
    });
    ...
    it('onChangeMM', () => {
      ...
      expect(onSetTime).toBeCalledWith(setMinutes(dt, 12));
    });

    it('onChangeSS', () => {
      const dt = new Date();
      const onSetTime = jest.fn();
      const wrapper = shallow(<TimeSelect value={dt} onSetTime={onSetTime} showSeconds />);
      wrapper
        .find('InputUpDown')
        .at(2)
        .props()
      ...
      expect(onSetTime).toBeCalledWith(setSeconds(dt, 7));
    });
    ...
    it('onChangeTableHH()', () => {
      const onSetTime = jest.fn();
      const wrapper = shallow(<TimeSelect value={new Date(2010, 9, 8)} onSetTime={onSetTime} />);
    ...
    it('onChangeTableSS()', () => {
      ...
      const wrapper = shallow(
        <TimeSelect value={new Date(2010, 9, 8)} onSetTime={onSetTime} showSeconds />
      );
